<template>
  <b-row class="qualification pr-5" style="position: relative">
    ...
    <b-col xl="6">
      <b-form-group
        :label="$t('validation.attributes.qualifications_info.university')"
        :invalid-feedback="feedback(qualification_authority_field)"
        :state="state(qualification_authority_field)"
      >
        <b-form-input
          :placeholder="
            $t('validation.attributes.qualifications_info.university')
          "
    ...
    <b-col xl="6">
      <b-form-group
        :label="$t('validation.attributes.qualifications_info.qualification')"
        :invalid-feedback="feedback(qualification_degree_field)"
        :state="state(qualification_degree_field)"
      >
        <b-form-input
          :placeholder="
            $t('validation.attributes.qualifications_info.qualification')
          "
...
<script>
export default {
  name: 'EmployeeAcademicQualification',
  props: {
    value: {
      default: () => ({
        id: null,
    ...
    compName: {
      default: 'academic_qualification',
      type: String,
      required: false
    }
  },
  computed: {
    qualification_authority_field: function() {
      return (
        'qualifications_info.academic_qualifications.' +
        this.index +
        '.authority'
      )
    },
    qualification_degree_field: function() {
      return (
        'qualifications_info.academic_qualifications.' + this.index + '.degree'
      )
    },
    qualification_year_field: function() {
      return (
        'qualifications_info.academic_qualifications.' + this.index + '.year'
      )
    },
    qualification_file_field: function() {
      return (
        'qualifications_info.academic_qualifications.' + this.index + '.file'
      )
    }
  },
  methods: {
